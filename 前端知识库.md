...
创建`.excalidraw`为后缀的文件

![image-20240908114317245](images/image-20240908114317245.png)
...
### 如何使用React.ReactChildren && typescript指定组件类型

Tab组件和TabContent组件

Tab组件的children指定需要TabContent组件，则`children: React.ReactElement<typeof TabContent>[]`

使用`React.Children.toArray(children).map`灵活封装组件
...
export default function MarkdownRender({ markdownText }) {
  return (
    <Box>
      <Markdown
...
## Mui Joy UI的Typography组件不渲染空格和换行符

添加`sx={{ whiteSpace: 'pre-wrap' }}`
...
## 使用flex布局总是容易超出屏幕宽度，如何解决

使用`overflow: "hidden"`防止内容溢出

如果还不能解决，需要在className="right-layout"添加

```react
flex: "0 0 50px", // 右侧布局固定为50px  
maxWidth: "50px" // 最大宽度为50px  
```
...
<Box sx={{
    ...
    <Box className="left-layout" sx={{
            flex: 1 // 左侧布局占用剩余空间 
        }}>
        <LeftLayout></LeftLayout>
    </Box>

    <Box className="right-layout" sx={{
